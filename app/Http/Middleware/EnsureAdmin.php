...
// Middleware que restringe el acceso exclusivamente a los administradores (id_rol = 1)
class EnsureAdmin
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ((int) $user->id_rol === 1) { // Verifica si el rol del usuario es administrador
            return $next($request); // Permite continuar con el siguiente middleware o controlador
        }

        if (in_array((int) $user->id_rol, [2, 3, 4], true)) { // Verifica si el rol del usuario es profesor, rector o vigilante
