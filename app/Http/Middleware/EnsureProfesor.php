...
// Middleware que restringe el acceso exclusivamente a los profesores (id_rol = 2)
class EnsureProfesor
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ((int) $user->id_rol === 2) { // Verifica si el rol del usuario es profesor
            return $next($request); // Permite continuar con el siguiente middleware o controlador
        }

        if (in_array((int) $user->id_rol, [1, 3, 4], true)) { // Verifica si el rol del usuario es admin, rector o vigilante
