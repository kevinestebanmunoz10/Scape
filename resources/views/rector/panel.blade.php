...
@section('content')
    ...
    <div class="dashboard-shell">
        {{-- Barra lateral de navegación del rector --}}
        <aside class="sidebar">
            ...
            <nav class="menu sidebar-account">
                {{-- Enlace al perfil del rector; se marca como activo según la ruta actual --}}
                <a href="{{ route('rector.perfil') }}" class="{{ request()->routeIs('rector.perfil') ? 'active' : '' }}">
            ...
            {{-- Menú principal de navegación del panel del rector --}}
            <nav class="menu sidebar-main">
                {{-- Acceso al dashboard; resalta el enlace si estamos en la ruta correspondiente --}}
                <a href="{{ route('rector.dashboard') }}" class="{{ request()->routeIs('rector.dashboard') ? 'active' : '' }}">
                ...
                {{-- Acceso a la gestión de usuarios; activo en cualquier ruta rector.usuarios.* --}}
                <a href="{{ route('rector.usuarios.index') }}" class="{{ request()->routeIs('rector.usuarios.*') ? 'active' : '' }}">
                    {{-- Ícono de usuarios --}}
                    <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="3.5"/><path d="M5 20c0-4 3-6 7-6s7 2 7 6"/></svg>
                    Usuarios
                </a>
                {{-- Enlace de accesos (aún sin destino definido para este rol) --}}
                <a href="#">
                ...
                {{-- Enlace de equipos (aún sin destino definido para este rol) --}}
                <a href="#">
