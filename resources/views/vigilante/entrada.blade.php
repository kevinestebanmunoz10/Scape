...
@section('title', 'Vigilante - Registrar ingreso')
...
@section('panel-content')
    ...
    {{-- Panel con el flujo de registro de ingreso --}}
    <section class="panel ingreso-panel">
        ...
        <div class="panel-header">
            <div>
                ...
                {{-- Descripción de uso --}}
                <span class="date-range">Busca a la persona y confirma su entrada</span>
            </div>
            {{-- Enlace para regresar al inicio del vigilante --}}
            <a href="{{ route('vigilante.dashboard') }}" class="filter-clear">Volver</a>
        </div>

        {{-- Si la sesión contiene una persona encontrada, muestra la confirmación de ingreso --}}
        @if (session('persona'))
            {{-- Recupera los datos de la persona desde la sesión --}}
            @php($persona = session('persona'))
            {{-- Tarjeta de confirmación del ingreso --}}
            <div class="confirm-card">
                ...
                {{-- Formulario que confirma y registra el ingreso --}}
                <form method="POST" action="{{ route('vigilante.entrada.store') }}">
                    ...
                    {{-- Botón que confirma el ingreso --}}
                    <button type="submit" class="date-apply btn-entrada">Confirmar ingreso</button>
                </form>
            </div>
        @else
            {{-- Formulario de búsqueda de la persona por tipo y documento --}}
            <form class="ingreso-form" method="POST" action="{{ route('vigilante.entrada.buscar') }}">
                {{-- Token CSRF de Laravel --}}
                @csrf
